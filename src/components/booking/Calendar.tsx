...
function getDayBackground(status: DayStatus, isSelected: boolean): string {
  if (isSelected) return COLORS.primary;
  if (status === 'past') return COLORS.disabled;
  if (status === 'blocked') return '#d0d0d0';
  if (status === 'full') return COLORS.full;
  if (status === 'has_bookings') return COLORS.hasBookings;
    ...
    <div style={{ background: 'rgba(255,255,255,0.95)', borderRadius: '16px', padding: '24px', marginBottom: '20px', boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px' }}>
        ...
        {Array(daysInMonth).fill(null).map((_, i) => {
          ...
          const bg = getDayBackground(status, isSelected);
          
          return (
            <button key={day} onClick={() => !isDisabled && onDateSelect(dateStr)} disabled={isDisabled}
              style={{ aspectRatio: '1', display: 'flex', alignItems: 'center', justifyContent: 'center',
                background: bg, border: `1px solid ${isSelected ? COLORS.primary : '#333'}`, borderRadius: '8px', color: isDisabled ? '#999' : '#333',
        ...
        <div style={{ marginTop: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
            {TIME_SLOTS.map(time => {
              ...
              return (
                <button key={time} onClick={() => !isBooked && onTimeSelect(time, selectedDate)} disabled={isBooked}
      ...
      <button onClick={() => {}} disabled={!selectedDate || !selectedTime}
