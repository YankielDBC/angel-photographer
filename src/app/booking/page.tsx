      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
        ...
        {currentStep === 2 && (
          <div style={{ background: 'rgba(255,255,255,0.95)', borderRadius: '16px', padding: '24px', marginBottom: '20px' }}>
            ...
            const canContinueStep2 = formData.packageType && formData.deliveryType && formData.packageTier && formData.name && formData.email && formData.phone;
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <button onClick={() => setCurrentStep(3)} disabled={!canContinueStep2}
                style={{ flex: 2, padding: '14px', background: canContinueStep2 ? '#c9a962' : '#333',
                  border: 'none', borderRadius: '12px', color: canContinueStep2 ? '#000' : '#666', fontSize: '14px', cursor: canContinueStep2 ? 'pointer' : 'not-allowed', minHeight: '48px' }}>
                Continuar
              </button>
            </div>
          </div>
