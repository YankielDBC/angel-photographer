        ...
        {selectedMonthBookings.length === 0 ? <p className="text-center text-gray-400 text-sm py-4">No hay reservas este mes</p> : (
          <div className="overflow-x-auto"><table className="w-full text-sm">
            ...
            <tbody>{selectedMonthBookings.map(b => { 
              ...
              return (
              <tr key={b.id} className="border-b border-gray-50 hover:bg-gray-50"><td className="py-2">{new Date(b.sessionDate).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' })}</td><td className="py-2 truncate max-w-[100px]">{b.client.name}</td><td className="py-2">{b.serviceTier}</td><td className="py-2 text-right">{display}</td><td className="py-2 text-right text-red-500">${(b.sessionCost || 0) + ((b.expenses || []).reduce((s: number, e: any) => s + (e.amount || 0), 0))}</td><td className="py-2 text-right font-medium">{isCompleted ? `$${b.totalAmount - ((b.sessionCost || 0) + ((b.expenses || []).reduce((s: number, e: any) => s + (e.amount || 0), 0)))}` : '-'}</td><td className="py-2 text-center"><span className={`text-xs px-2 py-0.5 rounded-full ${b.status === 'completed' ? 'bg-blue-100 text-blue-700' : b.status === 'confirmed' ? 'bg-green-100 text-green-700' : b.status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>{b.status}</span></td></tr>
            )})}</tbody>
          </table></div>
        )}
      </div>
